...
//出血特效实例化
    ...
    void CheckAnimation()
    {
        if (lastIsAttack != GetComponent<Animation>().IsPlaying("attack01") || lastIsDie!=GetComponent<Animation>().IsPlaying("die")||lastIsIdle!=GetComponent<Animation>().IsPlaying("idle")||lastIsTakeDamage!=GetComponent<Animation>().IsPlaying("takedamage")||lastIsWalk!=GetComponent<Animation>().IsPlaying("walk") )
        {
            TranscriptManager._instance.AddEnemyToSyncAnimation(this);// 把自身传递到管理器里面 统一进行动画的更新
            lastIsAttack = GetComponent<Animation>().IsPlaying("attack01");
            lastIsDie = GetComponent<Animation>().IsPlaying("die");
            lastIsIdle = GetComponent<Animation>().IsPlaying("idle");
            lastIsTakeDamage = GetComponent<Animation>().IsPlaying("takedamage"); 
            lastIsWalk = GetComponent<Animation>().IsPlaying("walk");
        }
    }
}
